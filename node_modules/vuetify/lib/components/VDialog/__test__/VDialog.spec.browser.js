...
describe('VDialog', () => {
  ...
  it('should wrap focus to the last element when Shift+Tab from the initial content wrapper', async () => {
    const model = ref(true);
    render(() => _createElementVNode("div", null, [_createElementVNode("button", {
      "data-testid": "outside"
    }, [_createTextVNode("Outside")]), _createVNode(VDialog, {
      "modelValue": model.value,
      "onUpdate:modelValue": $event => model.value = $event,
      "persistent": true
    }, {
    ...
    const last = screen.getByCSS('button[data-testid="last"]');
    const dialogContent = document.querySelector('.v-dialog .v-overlay__content');
